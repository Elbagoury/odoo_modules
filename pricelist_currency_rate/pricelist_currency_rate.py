...
class PricelistCurrencyRate(models.Model):
    ...
    currency_service_id = fields.Integer(string="Currency service ID")
        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            for rule in items:
                ...
                if price is not False:
                    price_limit = price
                    currency_rate = pricelist.currency_rate or 1
                    print "PRICE IS:"
                    print price
                    print currency_rate
                    price = price * (1.0+(rule.price_discount or 0.0)) * (currency_rate)
                    if rule.price_round:
...
class UpdateService(models.Model):
    ...
    pricelists_ids = fields.Many2many('product.pricelist', 'currency_service_id', string="Pricelists to be updated")
    ...
    def run_test(self, cr, uid, ids, context=None):
        ...
        if rec:
            ...
            for pl in rec.pricelists_ids:
                name = pl.currency_id.name
                for x in rec.currency_to_update:
                    if name == x.name:
                        print "CHANGING"
                        pl.currency_rate = x.rate_silent
    ...
    def run_currency_update(self):
        ...
        self.run_test(self._cr, self._uid, self._ids, context=None)
